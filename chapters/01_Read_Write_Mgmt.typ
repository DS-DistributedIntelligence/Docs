...
As a requirement of the project, reads must be locally performed, while writes have to be approved by the majority, and only then the coordinator can proceed to apply and broadcast the change. The database is a map of `<idx: Integer, value: Integer>`, and each replica affects only its own database.
...
- the quorum is always available, meaning that a write request is always processed by the coordinator, if the latter does not crash in the meanwhile;
