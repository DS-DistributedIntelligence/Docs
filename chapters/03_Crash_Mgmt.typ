...
Checks for crash are set at the start of `onElectionStartedMsg`, `checkHeartbeatMsgStatus` and `sendSynchronizationMsg` methods, all invoking the `crashNow` method if they need to crash.