...
To simulate crashes during the election, the `Crash` class is present. Specifically, it is possible to crash the replica:
...
Checks for crashes are set at the start of `onElectionStartedMsg`, `checkHeartbeatMsgStatus` and `sendSynchronizationMsg` methods, all invoking the `crashNow` method if they need to crash.